...
- **Persistent mode (rev 3.5, D15, iOS only):** `continuous = true`, `interimResults = true`, one session per app layout.
  ...
  - **🔊 and lost capture (rev 3.9, device probe 2026-09-25, supersedes rev 3.6–3.8):**
    - **Before 🔊 plays**, the session is stopped gracefully (`stop()`, no window kept). The next mic tap opens a new session. Probe `strategy=stop`: that new session heard immediately, 4 answers in a row, and playback ran at full volume because the mic was off.
