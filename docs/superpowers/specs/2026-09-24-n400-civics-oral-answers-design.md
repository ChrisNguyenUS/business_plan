...
- **Persistent mode (rev 3.5, D15, iOS only):** `continuous = true`, `interimResults = true`, one session per app layout.
  ...
  - **After 🔊 (rev 3.7, device log 2026-09-25, replaces the rev 3.6 warm-up):** `<audio>` playback silently stops the running session's capture source. There are no results until a late `audio-capture: Source is stopped`, while a session opened after playback hears at once. Every 🔊 therefore marks the session, and the next mic tap starts a fresh one. The mic is never opened just because audio plays. The rev 3.6 warm-up is removed: it didn't prevent this, and it lowered playback volume.
