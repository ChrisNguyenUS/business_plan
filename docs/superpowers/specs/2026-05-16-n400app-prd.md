...
| 5 | Audio source for v1 | Owner | **RESOLVED** — owner personally recorded all MP3s under `apps/website/N400_voice/` (128 questions + 79 canonical answers + 100 Q23 senators + 56 Q61 governors + 50 Q62 capitals + 441 Q29 reps). Total ~854 files. No TTS in v1. Phase 2 = upload + DB backfill only. |
...
- **Phase 2:** All audio is owner-recorded under `apps/website/N400_voice/` — no TTS generation in v1. Phase 2 is a single upload + DB-backfill pipeline (manifest builder + upload script). v1 covers questions, canonical answers, Q23 senators, Q29 reps, Q61 governors (50 states + 6 territories), and Q62 capitals.
