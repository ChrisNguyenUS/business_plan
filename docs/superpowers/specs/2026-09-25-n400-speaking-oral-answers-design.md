...
- `transcriptStems(text, { keep })` keeps a stall phrase only when **every** one of its words (stopwords aside) is in `keep`. One shared word is not enough: "one more time" stays dropped for Civics Q35 (`more people`), keeping the rev 3.16 fix (S1 prototyping). Only **filler** stalls (asking for time, greetings, acknowledgements) can be kept; thinking-aloud, not-knowing and asking-again stalls are never an answer and are always dropped (S1 final review: "I need to think" kept `need` and graded near What-mean #52 through its synonym `not need`).
