...
- [x] **Website Phase 3E — N400 Growth Engine G2 (conversation model)** — Progressive profiling live behind `profiling`+`growth_engine` flags: pure evaluator (`growth/profiling.ts`, surface routing + snooze-as-active-days + deterministic variants + debug reason), answer/skip/mark-shown SECURITY DEFINER RPCs with journey-stage derivation (n400_21), insert-path event emitter fix (n400_20), full `prompt_shown → answered/skipped` funnel tagged by question × variant × surface, Level 2 card under Practice/Mock results, Level 1 dashboard soft card, Level 3 interview-mode hero intent via the priority-ordered `GROWTH_INTENT_TIERS` table. Spec §3. G3c (filing checklist), G4 (internal_app Leads) pending.
- [x] **Website Phase 3E — N400 Growth Engine G3a (CTA engine)** — Behavior-driven CTA live behind `cta_engine`+`growth_engine` flags: pure evaluator (`growth/cta.ts`) over the 8 G1-seeded scenarios with all seven §4.1 hard rules (7-day cap, per-CTA cooldown, dismiss→snooze, 3-dismiss→30-day group mute, converted→consultation retired, priority order, action availability), server-side learning signals reusing the client's own `deriveReadiness`/`deriveSectionGradedTally` (drift-locked by test), shown/dismiss/click RPCs + `n400_cta_decision_log` with 30-day probabilistic retention (n400_24), `cta_*` events moved server-only, and `getGrowthState` collapsing prompt+CTA into one read. Spec §4. G3c (filing checklist), G4 (internal_app Leads) pending.
- [x] **Website Phase 3E — N400 Growth Engine G3b (booking flow)** — Consultation booking live behind `booking_form`+`growth_engine` flags (spec §5): pure `booking.ts` validation + CTA-topic prefill, `n400_25` SQL rollup RPCs (`n400_learning_rollup`, `n400_graded_day_rollup`) so `growth-context.ts`/`learning-signals.ts` stop hauling one-row-per-answer tables through PostgREST's silent 1000-row cap, `booking-actions.ts` server action (own-user insert + dedupe on an existing open request + best-effort Resend staff notify via `notify.ts` + Meta CAPI `Lead`, side effects never block the insert), `/n400ready/(app)/consultation` page (name/phone/topic/preferred-time form with `getDisplayName()` prefill, confirm screen, Calendly handoff), and CTA gating so a pending consultation request suppresses the `book_consultation` CTA group. Also fixes the G3a post-merge review's S1 boundary bug (`cta.ts` `min_avg_pct` now inclusive). Spec §5. G3c (filing checklist), G4 (internal_app Leads) pending.
