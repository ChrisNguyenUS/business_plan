    ...
    <div className="py-16 lg:py-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 p-5 rounded-2xl bg-amber-50 border border-amber-200 flex gap-3">
          ...
          <div>
            <p className="font-semibold text-amber-900 text-sm mb-1">Important Disclosure / Thông Báo Quan Trọng</p>
            <p className="text-amber-800 text-sm leading-relaxed">
              Manna One Solution is a <strong>non-attorney document preparer</strong>. We prepare USCIS forms at your direction. We are NOT a law firm and do NOT provide legal advice. We do NOT file Form G-28 or represent clients before USCIS. For complex cases (RFEs, prior denials, criminal history), we refer you to a licensed immigration attorney.
            </p>
        ...
        {/* Header */}
        <div className="mb-10">
          <h1 className="text-3xl lg:text-5xl font-bold text-charcoal mb-4">Immigration Services / Dịch Vụ Di Trú</h1>
          <p className="text-muted-foreground text-lg leading-relaxed max-w-2xl">Vietnamese-language USCIS document preparation for Texas residents. Non-attorney document preparer — transparent pricing, bilingual service.</p>
          <p className="text-primary font-semibold mt-2 text-sm">Starting from $50 · I-90 green card renewal from $715 total · Free consultation</p>
        </div>
        ...
        <div className="mb-12">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            All prices = Service Fee + USCIS Filing Fee. USCIS fees verified as of April 2026 at{' '}
            <a href="https://www.uscis.gov/forms/filing-fees" target="_blank" rel="noopener noreferrer" className="text-primary underline">uscis.gov/forms/filing-fees</a>.
            Service fee is flat; USCIS fee is subject to change.
          </p>
          <div className="bg-white rounded-2xl border border-border overflow-hidden">
            {services.map((s, i) => (
              <div key={i} className={`flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 py-4 gap-1 ${i < services.length - 1 ? 'border-b border-border' : ''}`}>
                <div>
        ...
        <div className="mb-12 p-6 rounded-2xl bg-red-50 border border-red-100">
          ...
          <ul className="text-red-800 text-sm space-y-1">
            {[
              ...
              <li key={i} className="flex items-start gap-2"><span className="mt-0.5 text-red-500">✗</span> {item}</li>
            ))}
          </ul>
          <p className="text-red-800 text-sm mt-3">
            For complex cases, we refer clients to:{' '}
            <a href="https://www.cliniclegal.org" target="_blank" rel="noopener noreferrer" className="underline">CLINIC</a>,{' '}
            <a href="https://www.supportkind.org" target="_blank" rel="noopener noreferrer" className="underline">KIND</a>,
        ...
        {/* FAQ */}
        <div className="mb-12">
          <h2 className="text-xl font-bold text-charcoal mb-6">Câu Hỏi Thường Gặp / Frequently Asked Questions</h2>
          <Accordion type="single" collapsible className="w-full">
            {faqs.map((faq, i) => (
              <AccordionItem key={i} value={`faq-${i}`}>
                <AccordionTrigger className="text-left text-charcoal font-medium">{faq.q}</AccordionTrigger>
        ...
        <div className="text-center p-10 rounded-2xl bg-charcoal">
          <h3 className="text-2xl font-bold text-white mb-2">Đặt Lịch Tư Vấn Miễn Phí / Book Free Consultation</h3>
          <p className="text-white/60 text-sm mb-6">For Texas residents only. Dành cho cư dân Texas.</p>
          <Link to="/contact">
