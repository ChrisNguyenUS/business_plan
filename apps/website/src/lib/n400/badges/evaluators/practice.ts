...
async function allMockSessions(userId: string, supabase: SupabaseClient): Promise<MockSession[]> {
...
const practiceExamReady: BadgeEvaluator = async (userId, ctx, supabase) => {
  const { civics } = await passedMockCounts(userId, supabase);
...
const practiceHighScore: BadgeEvaluator = async (userId, ctx, supabase) => {
  const sessions = await allMockSessions(userId, supabase);
