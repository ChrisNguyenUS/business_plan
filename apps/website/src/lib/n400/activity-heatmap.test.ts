...
import { buildHeatGrid, heatWeekdays } from './activity-heatmap';
...
describe('buildHeatGrid', () => {
  ...
  it('scales intensity by the number of attempts in a day', () => {
    ...
    const cell = (times: string[]) => buildHeatGrid(at(times), NOW, vi).grid[4][6];
  ...
  it('counts distinct active days, not attempts', () => {
    const { totalDays } = buildHeatGrid(at([daysAgo(0), daysAgo(0), daysAgo(3)]), NOW, vi);
  ...
  it('names the most-studied weekday', () => {
    ...
    const { busiestDay } = buildHeatGrid(at([daysAgo(0), daysAgo(7), daysAgo(1)]), NOW, vi);
    expect(busiestDay).toBe('T4');
    expect(heatWeekdays(vi)).toContain(busiestDay);
  });
  ...
  it('merges attempts from every skill onto one calendar', () => {
    ...
    expect(buildHeatGrid([...civics, ...sections], NOW, vi).grid[4][6]).toBe(1); // 3 attempts → level 1
    expect(buildHeatGrid([...civics, ...sections, ...at([daysAgo(0)])], NOW, vi).grid[4][6]).toBe(2); // 4 → level 2
  });

  it('ignores activity older than the 35-day window in the grid', () => {
    const { grid, totalDays } = buildHeatGrid(at([daysAgo(200)]), NOW, vi);
    expect(grid.flat().every((v) => v === 0)).toBe(true);
